...
var XSD = 'http://www.w3.org/2001/XMLSchema#',
    XSD_INTEGER = Literal.INTEGER = XSD + 'integer',
    XSD_DOUBLE  = Literal.DOUBLE  = XSD + 'double',
    XSD_BOOLEAN = Literal.BOOLEAN = XSD + 'boolean',
    XSD_STRING  = Literal.STRING  = XSD + 'string',
    LANG_STRING = Literal.LANG_STRING = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#langString';
...
// ### Constructs an ID from the given term or ID string
Term.toId = function (term) {
  ...
  case 'Literal':      return '"' + term.value + '"' +
    (term.language ? '@' + term.language :
      (term.datatype && term.datatype.value !== XSD_STRING ? '^^' + term.datatype.value : ''));
...
function literal(value, languageOrDataType) {
  ...
  if (!modifier) {
    switch (typeof value) {
    case 'boolean':
      modifier = XSD_BOOLEAN;
      break;
    case 'number':
      if (isFinite(value))
        modifier = value % 1 === 0 ? XSD_INTEGER : XSD_DOUBLE;
      else {
        modifier = XSD_DOUBLE;
        if (!isNaN(value))
