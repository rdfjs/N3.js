...
N3Store.prototype = {
        ...
        for (var value1 in index1) {
          ...
          if (index2 = index1[value1]) {
            ...
            for (var l = values.length - 1; l >= 0; l--) {
              var result = new Quad(null, null, null, Term.fromId(graph));
              result[name0] = Term.fromId(entity0);
              result[name1] = Term.fromId(entity1);
              result[name2] = Term.fromId(entityKeys[values[l]]);
              if (array)
                array.push(result);
  ...
  _uniqueEntities: function (callback) {
    ...
    return function (id) {
      if (!(id in uniqueIds)) {
        uniqueIds[id] = true;
        callback(Term.fromId(entities[id]));
      }
    };
  },
  ...
  // ### `addTriple` adds a new triple to the store.
  ...
  addTriple: function (subject, predicate, object, graph) {
    ...
    subject = Term.toId(subject);
    predicate = Term.toId(predicate);
    object = Term.toId(object);
    graph = Term.toId(graph);
  ...
  // ### `addTriples` adds multiple triples to the store
  ...
  import: function (stream) {
    var self = this;
    stream.on('data', function (quad) { self.addTriple(quad); });
    return stream;
  },

  // ### `removeTriple` removes a triple from the store if it exists
  ...
  // ### `removeTriples` removes multiple triples from the store
  ...
  remove: function (stream) {
    var self = this;
    stream.on('data', function (quad) { self.removeTriple(quad); });
    return stream;
  },

  // ### `getTriples` returns an array of triples matching a pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  getTriples: function (subject, predicate, object, graph) {
    // Convert terms to internal string representation
    subject = subject && Term.toId(subject);
    predicate = predicate && Term.toId(predicate);
    object = object && Term.toId(object);
    graph = graph && Term.toId(graph);
  ...
  // ### `countTriples` returns the number of triples matching a pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  ...
  createBlankNode: function (suggestedName) {
    ...
    return new BlankNode(name.substr(2));
  },
};
