...
N3Store.prototype = {
  ...
  // ### `_tripleDigest` returns a string that is the same for
  // identical triples (regardless of their graph) and different
  // otherwise.
  _tripleDigest: function(triple) {
  ...
  // ### `findAllGraphs` is like find() but searches all graphs, and
  // the result objects have `graphs`: [...] instead of a `graph` attribute.
  findAllGraphs: function (subject, predicate, object) {
      var combined = [];
      var seenDigests = {}; // digest : output row
      for (var graphKey in this._graphs) {
          var triples = this.find(subject, predicate, object, graphKey);
          for (var i = triples.length - 1; i >= 0; i--) {
              var triple = triples[i];
              var digest = this._tripleDigest(triple);
              if (seenDigests[digest]) {
                  seenDigests[digest].graphs.push(triple.graph);
              } else {
                  var newRow = {
