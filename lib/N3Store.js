...
class N3Store {
  constructor(quads, options) {
    ...
    // '_changeStream' holds active streams created by this.match()
    this._changeStream = new N3Stream();
  ...
  addQuad(subject, predicate, object, graph) {
    ...
    var newQuad = quad(subject, predicate, object, graph);
    ...
    this._changeStream.emit('data', newQuad);
  ...
  removeQuad(subject, predicate, object, graph) {
    ...
    var removedQuad = quad(subject, predicate, object, graph);
    ...
    this._changeStream.emit('remove', removedQuad);
  ...
  match(subject, predicate, object, graph) {
    var stream = new N3Stream();
    var compareQuad = quad(toId(subject), toId(predicate), toId(object), toId(graph));

    function matchQuad(quad) {
      return (
        (subject === undefined || quad.subject === compareQuad.subject) &&
        (predicate === undefined || quad.predicate === compareQuad.predicate) &&
        (object === undefined || quad.object === compareQuad.object) &&
        (graph === undefined || quad.graph === compareQuad.graph)
      );
    }

    this._changeStream.on('data', function (quad) {
      if (matchQuad(quad))
        stream.emit('data', quad);
    });
    this._changeStream.on('remove', function (quad) {
      if (matchQuad(quad))
        stream.emit('remove', quad);
    });
    this._changeStream.on('end', function () {
      stream.emit('end');
    });
    return stream;
  }
