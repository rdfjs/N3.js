...
N3Store.prototype = {
  ...
  find: function (subject, predicate, object, context) {
    ...
    if ((match = prefixMatcher.exec(subject))   !== null && (base = prefixes[prefix = match[1]]))
      subject   =   subject.substr(0, match.index) + base +   subject.substr(prefix.length + match.index + 1);
    if ((match = prefixMatcher.exec(predicate)) !== null && (base = prefixes[prefix = match[1]]))
      predicate = predicate.substr(0, match.index) + base + predicate.substr(prefix.length + match.index + 1);
    if ((match = prefixMatcher.exec(object))    !== null && (base = prefixes[prefix = match[1]]))
      object    =    object.substr(0, match.index) + base +    object.substr(prefix.length + match.index + 1);
