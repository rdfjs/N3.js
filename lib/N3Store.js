...
N3Store.prototype = {
  ...
  // ### `_findGraphs` returns the graph with the given name,
  // or all graphs if the argument is null or undefined.
  _findGraphs: function (graph) {
    if (!isString(graph))
      return this._graphs;
    var graphs = {};
  ...
  findByIRI: function (subject, predicate, object, graph) {
    var quads = [], graphs = this._findGraphs(graph), content,
  ...
  countByIRI: function (subject, predicate, object, graph) {
    var count = 0, graphs = this._findGraphs(graph), content,
        ids = this._ids, subjectId, predicateId, objectId;
  ...
  someByIRI: function (callback, subject, predicate, object, graph) {
    var graphs = this._findGraphs(graph), content,
        ids = this._ids, subjectId, predicateId, objectId;
  ...
  forSubjectsByIRI: function (callback, predicate, object, graph) {
    var ids = this._ids, graphs = this._findGraphs(graph), content, predicateId, objectId;
  ...
  forPredicatesByIRI: function (callback, subject, object, graph) {
    var ids = this._ids, graphs = this._findGraphs(graph), content, subjectId, objectId;
    callback = this._uniqueEntities(callback);
  ...
  forObjectsByIRI: function (callback, subject, predicate, graph) {
    var ids = this._ids, graphs = this._findGraphs(graph), content, subjectId, predicateId;
    callback = this._uniqueEntities(callback);
