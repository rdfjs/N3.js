...
// ## Constructor
function N3Writer(outputStream, prefixes, options) {
  if (!(this instanceof N3Writer))
    return new N3Writer(outputStream, prefixes, options);

  // Shift arguments if the first argument is not a stream
  if (outputStream && typeof outputStream.write !== 'function')
    options = prefixes, prefixes = outputStream, outputStream = null;
  ...
  this._subject = null;
  if (!options || !(/triple|quad/i).test(options.format)) {
    this._graph = '';
...
function characterReplacer(character) {
  ...
  if (character.length === 1) return escapeReplacements[character];
  // Replace a surrogate pair with its unicode escape sequence
  var result = ((character.charCodeAt(0) - 0xD800) << 10 |
                 character.charCodeAt(1) + 0x2400).toString(16);
  return '\\U00000000'.substr(0, 10 - result.length) + result;
}
