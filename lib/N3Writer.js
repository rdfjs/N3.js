...
N3Writer.prototype = {
  ...
  // ### `_writeTripleTerms` writes the triple term objects to the output stream
  _writeTripleTerms: function (subject, predicate, object, graph, done) {
    this._writeTriple(Term.toId(subject), Term.toId(predicate), Term.toId(object), Term.toId(graph), done);
  },
  ...
  tripleToString: function (subject, predicate, object, graph) {
    return  this._encodeIriOrBlankNode(Term.toId(subject))   + ' ' +
            this._encodeIriOrBlankNode(Term.toId(predicate)) + ' ' +
            this._encodeObject(Term.toId(object)) +
            (graph && (typeof graph === 'string' || graph.value) ? ' ' + this._encodeIriOrBlankNode(Term.toId(graph)) + '.\n' : '.\n');
  },
  ...
  addTriple: function (subject, predicate, object, graph, done) {
    ...
    // The optional `graph` parameter was not provided
      ...
      this._writeTripleTerms(subject, predicate, object, new DefaultGraph(), graph);
    // The `graph` parameter was provided
    else
      this._writeTripleTerms(subject, predicate, object, graph, done);
  },
  ...
  blank: function (predicate, object) {
    ...
    case 1:
      child = children[0];
      if (child.object[0] !== '[')
        return '[ ' + this._encodePredicate(Term.toId(child.predicate)) + ' ' +
                      this._encodeObject(Term.toId(child.object)) + ' ]';
    ...
    default:
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (Term.toId(child.predicate) === predicate)
          contents += ', ' + this._encodeObject(Term.toId(child.object));
        // Otherwise, write the predicate and the object
        else {
          contents += (i ? ';\n  ' : '\n  ') +
                      this._encodePredicate(Term.toId(child.predicate)) + ' ' +
                      this._encodeObject(Term.toId(child.object));
          predicate = Term.toId(child.predicate);
        }
      }
      return contents + '\n]';
    }
  ...
  list: function (elements) {
    ...
    for (var i = 0; i < length; i++)
      contents[i] = this._encodeObject(Term.toId(elements[i]));
