...
function N3Parser(config) {
  ...
  var format = (typeof config.format === 'string') && config.format.match(/\w*$/)[0].toLowerCase(),
      isTurtle = format === 'turtle', isTriG = format === 'trig';
  if (!(this._supportsNamedGraphs = !isTurtle))
    this._readPredicateOrNamedGraph = this._readPredicate;
  this._supportsQuads = !(isTurtle || isTriG);
    ...
    case 'type':
      ...
      if (token.prefix === '') {
        if (this._baseIRI === null || absoluteIRI.test(token.value))
          ...
          value = this._resolveIRI(token.value);
      }
      else {
        var prefix = this._prefixes[token.prefix];
        if (prefix === undefined)
    ...
    case 'IRI':
      if (this._supportsQuads && this._graph === null) {
        if (this._baseIRI === null || absoluteIRI.test(token.value))
          ...
          graph = this._resolveIRI(token.value);
