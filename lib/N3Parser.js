...
function N3Parser(config) {
  ...
  this._blankPrefix = '_:b' + blankNodeCount++ + '_';
  this._graph = null;

  // Set supported features depending on the format.
  var format = this._format = (typeof config.format === 'string') &&
                              config.format.match(/\w*$/)[0].toLowerCase() || 'hybrid';
  if (!(this._supportsNamedGraphs = (format !== 'turtle')))
    this._readPredicateOrNamedGraph = this._readPredicate;

  // Set the document IRI.
  if (!config.documentIRI) {
    this._baseIRI = null;
    this._baseIRIPath = null;
  }
  else {
    if (config.documentIRI.indexOf('#') > 0)
      throw new Error('Invalid document IRI');
    this._baseIRI = config.documentIRI;
...
N3Parser.prototype = {
  ...
  _readSubject: function (token) {
    ...
    case 'IRI':
      if (this._baseIRI === null || absoluteIRI.test(token.value))
        ...
        this._subject = this._resolveIRI(token.value);
      break;
    case 'prefixed':
      if (token.prefix === '_') {
        this._subject = this._blankPrefix + token.value;
      }
      else {
  ...
  _readPredicate: function (token) {
    ...
    case 'abbreviation':
      if (this._baseIRI === null || absoluteIRI.test(token.value))
        ...
        this._predicate = this._resolveIRI(token.value);
      break;
    case 'prefixed':
      if (token.prefix === '_') {
  ...
  _readObject: function (token) {
    ...
    case 'IRI':
      if (this._baseIRI === null || absoluteIRI.test(token.value))
        ...
        this._object = this._resolveIRI(token.value);
      break;
    case 'prefixed':
      if (token.prefix === '_') {
        this._object = this._blankPrefix + token.value;
      }
      else {
        var prefix = this._prefixes[token.prefix];
        if (prefix === undefined)
  ...
  _readListItem: function (token) {
    ...
    case 'prefixed':
      if (token.prefix === '_') {
        item = this._blankPrefix + token.value;
      }
      else {
        var prefix = this._prefixes[token.prefix];
        if (prefix === undefined)
  ...
  _readPrefixIRI: function (token) {
    ...
    if (this._baseIRI === null || absoluteIRI.test(token.value))
      ...
      prefixIRI = this._resolveIRI(token.value);
  ...
  _readBaseIRI: function (token) {
    ...
    if (this._baseIRI === null || absoluteIRI.test(token.value))
      ...
      this._baseIRI = this._resolveIRI(token.value);
    this._baseIRIPath = this._baseIRI.replace(documentPart, '');
    this._baseIRIRoot = this._baseIRI.match(rootIRI)[0];
    return this._readDeclarationPunctuation;
  },
  ...
  // ### `_resolveIRI` resolves an IRI against a base path
