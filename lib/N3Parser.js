...
function N3Parser(config) {
  ...
  if (!config.documentURI) {
    this._baseURI = this._documentURI = null;
    this._baseURIROOT = this._documentURIRoot = null;
  }
  else {
    this._baseURI = this._documentURI = config.documentURI;
    this._baseURIRoot = this._documentURIRoot = config.documentURI.replace(documentPart, '');
  }
}
