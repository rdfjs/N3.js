...
N3Parser.prototype = {
  ...
  _resolveIRI: function (token) {
    ...
    case undefined: return this._baseIRI;
    // Resolve relative fragment IRIs against the base IRI
    case '#': return this._baseIRI + iri;
    // Resolve relative query string IRIs by replacing the query string
    case '?': return this._baseIRI.replace(/(?:\?.*)?$/, iri);
    // Resolve root relative IRIs at the root of the base IRI
    case '/': return this._baseIRIRoot + iri;
    // Resolve IRIs starting with sequences of './' and/or '../' against parent paths of the base IRI
    case '.':
      // Start resolving from the base IRI's path
      var basePath = this._baseIRIPath, basePos = basePath.length, rootLength = this._baseIRIRoot.length,
          iriPos = 0, matchLength;
      // Identify the next trailing sequence
      do
        switch (iri[iriPos + 1]) {
          // Match a trailing './'
          case '/':
            matchLength = 2;
            break;
          // Match a trailing '../', and take the base IRI's parent path (unless it's already at the root)
          case '.':
            if (iri[iriPos + 2] === '/') {
              matchLength = 3;
              basePos = Math.max(basePath.lastIndexOf('/', basePos - 2), rootLength);
              break;
            }
          // Other trailing sequences do not match
          default:
            matchLength = 0;
        }
      // If there was a match, advance past that match and continue resolving if potential matches remain
      while (matchLength !== 0 && iri[iriPos += matchLength] === '.');
      // Combine the remainder of the base path with the remainder of the relative IRI
      return basePath.substr(0, basePos + 1) + iri.substr(iriPos);
    // Resolve all other IRIs at the base IRI's path
    default: return this._baseIRIPath + iri;
    }
  },
