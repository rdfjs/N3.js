...
var RDF_PREFIX  = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#',
    ...
    RDF_NIL     = namedNode(RDF_PREFIX + 'nil'),
    RDF_FIRST   = namedNode(RDF_PREFIX + 'first'),
    RDF_REST    = namedNode(RDF_PREFIX + 'rest'),
    N3_FORSOME  = namedNode(SWAP_PREFIX + 'reify#forSome'),
    N3_FORALL   = namedNode(SWAP_PREFIX + 'reify#forAll'),
    ABBREVIATIONS = {
      'a': namedNode(RDF_PREFIX + 'type'),
      '=': namedNode('http://www.w3.org/2002/07/owl#sameAs'),
      '>': namedNode(SWAP_PREFIX + 'log#implies'),
    },
    DEFAULTGRAPH = defaultGraph(),
    QUANTIFIERS_GRAPH = namedNode('urn:n3:quantifiers');
...
N3Parser.prototype = {
  ...
  _blankNode: function () {
    return blankNode('b' + blankNodeCount++);
  },
  ...
  _readEntity: function (token, quantifier) {
    ...
    case 'IRI':
    case 'typeIRI':
      value = namedNode(this._base === null || absoluteIRI.test(token.value) ?
    ...
    case 'prefixed':
      ...
      value = namedNode(prefix + token.value);
      break;
    // Read a blank node
    case 'blank':
      value = blankNode(this._prefixes[token.prefix] + token.value);
      break;
    // Read a variable
    case 'var':
      value = variable(token.value.substr(1));
      break;
  ...
  _readObject: function (token) {
    ...
    case 'literal':
      ...
      // Pre-datatyped string literal (prefix stores the datatype)
        ...
        this._object = literal(token.value, namedNode(token.prefix));
      break;
    case '[':
      // Start a new quad with a new blank node as subject
  ...
  _readListItem: function (token) {
    ...
    case 'literal':
      ...
      // Pre-datatyped string literal (prefix stores the datatype)
        ...
        item = literal(token.value, namedNode(token.prefix));
        next = this._getContextEndReader();
      }
      break;
    default:
  ...
  // ### `_completeLiteral` completes a literal with an optional data type or language
  _completeLiteral: function (token, listItem) {
    ...
    case 'typeIRI':
      ...
      this._object = literal(value, datatype);
      token = null;
      break;
    // Create a language-tagged string
    case 'langcode':
      this._object = literal(value, token.value);
      token = null;
      break;
  ...
  _readPrefixIRI: function (token) {
    ...
    this._prefixCallback(this._prefix, namedNode(prefixIRI));
  ...
  // ### `_quad` emits a quad through the callback
  _quad: function (subject, predicate, object, graph) {
    this._callback(null, quad(subject, predicate, object, graph || DEFAULTGRAPH));
  },
