...
function N3Parser(options) {
  ...
  options = options || {};
  this._setBaseIRI(options.documentIRI);
...
N3Parser.prototype = {
  ...
  // ### `_setBaseIRI` sets the base IRI to resolve relative IRIs.
  _setBaseIRI: function (baseIRI) {
    if (!baseIRI)
      baseIRI = null;
    else if (baseIRI.indexOf('#') > 0)
      throw new Error('Invalid base IRI ' + baseIRI);
    // Set base IRI and its components
    if (this._baseIRI = baseIRI) {
      baseIRI = this._removeDotSegments(baseIRI.replace(/\?.*/, ''));
      this._baseIRIPath   = baseIRI.replace(documentPart, '');
      baseIRI = baseIRI.match(schemeAuthority);
      this._baseIRIRoot   = baseIRI[0];
      this._baseIRIScheme = baseIRI[1];
    }
  },
  ...
  _readBaseIRI: function (token) {
    ...
    try {
      this._setBaseIRI(this._baseIRI === null || absoluteIRI.test(token.value) ?
  ...
  _resolveIRI: function (token) {
    ...
    case '/':
      ...
      return (iri[1] === '/' ? this._baseIRIScheme : this._baseIRIRoot) + this._removeDotSegments(iri);
    // Resolve all other IRIs at the base IRI's path
    default:
      return this._removeDotSegments(this._baseIRIPath + iri);
    }
  },
  ...
  _removeDotSegments: function (iri) {
    ...
    while (i < length) {
      switch (next) {
      // The path starts with the first slash after the IRI scheme and authority
      case ':':
        if (pathStart < 0) {
          // Skip slashes directly after the scheme
          while (++i < length && iri[i] === '/');
          // Skip to first slash character
          while (++i < length && iri[i] !== '/');
          pathStart = i;
        }
        break;
        ...
        if (iri[i + 1] === '.') {
          ...
          case '.':
            ...
            if (next === undefined || next === '/' || next === '?' || next === '#') {
              ...
              if ((segmentStart = result.lastIndexOf('/', result.length - 2)) >= pathStart)
