...
N3Parser.prototype = {
  ...
  _readInTopContext: function (token) {
    ...
    // It could be a forSome statement
    case '@forSome':
      this._quantifiedPrefix = '_:b';
      return this._readQuantifierList;
    // It could be a forAll statement
    case '@forAll':
  ...
  _readSubject: function (token) {
    ...
    // In N3 mode, replace the subject if it is quantified
    if (this._n3Mode && (this._subject in this._quantified))
      this._subject = this._quantified[this._subject];
  ...
  _readPredicate: function (token) {
    ...
    // In N3 mode, replace the predicate if it is quantified
    if (this._n3Mode && (this._predicate in this._quantified))
      this._predicate = this._quantified[this._predicate];
  ...
  _readObject: function (token) {
    ...
    // In N3 mode, replace the object if it is quantified
    if (this._n3Mode && (this._object in this._quantified))
      this._object = this._quantified[this._object];
  ...
  _readDataTypeOrLang: function (token) {
    ...
    case 'typeIRI':
      if (this._base === null || absoluteIRI.test(token.value))
        ...
        suffix = '^^' + this._resolveIRI(token);
      break;
    case 'type':
      var prefix = this._prefixes[token.prefix];
      if (prefix === undefined)
        return this._error('Undefined prefix "' + token.prefix + ':"', token);
      suffix = '^^' + prefix + token.value;
      break;
  ...
  _readQuantifierList: function (token) {
    ...
    case 'IRI':
      if (this._base === null || absoluteIRI.test(token.value))
        ...
        entity = this._resolveIRI(token);
      break;
    case 'prefixed':
      var prefix = this._prefixes[token.prefix];
      if (prefix === undefined)
        return this._error('Undefined prefix "' + token.prefix + ':"', token);
      entity = prefix + token.value;
      break;
    default:
  ...
  parse: function (input, tripleCallback, prefixCallback) {
    ...
    this._subject = this._predicate = this._object = null;
    this._sparqlStyle = false;
