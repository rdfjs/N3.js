...
N3Store.prototype = {
  ...
  */
  _findInIndex: function(index0, key0, key1, key2, name0, name1, name2, context) {
    var results = [],
        ...
        tmp;
    // if a key is specified, use only that part of index 0, if it exists
    if (key0)
      tmp = {}, tmp[key0] = index0[key0], index0 = tmp;

    for(var value0 in index0) {
      var index1 = index0[value0] || {},
          entity0 = entityKeys[value0];
      // if a key is specified, use only that part of index 1, if it exists
      if (key1)
        tmp = {}, tmp[key1] = index1[key1], index1 = tmp;
      ...
      for (var value1 in index1) {
        ...
        if (key2)
          tmp = {}, key2 in index2 && (tmp[key2] = index2[key2]), index2 = tmp;
  ...
  */
  find: function (subject, predicate, object, context) {
    var context = context || this.defaultContext,
        contextItem = this._contexts[context],
